...
contextBridge.exposeInMainWorld('api', {
  ...
  extractAudioFromVideoToFile: (path: string) => ipcRenderer.invoke('extract-audio-from-video-to-file', path) as Promise<{ path: string; name: string }>,
  generateLtcWav: (opts: { startTC: string; durationSec: number; fps: number; amplitude?: number }) =>
    ipcRenderer.invoke('generate-ltc-wav', opts) as Promise<{ path: string; name: string }>,
  ...
  saveFileBuffer: (defaultName: string, filters: { name: string; extensions: string[] }[], buffer: ArrayBuffer) =>
    ipcRenderer.invoke('save-file-buffer', defaultName, filters, buffer) as Promise<string | null>,
