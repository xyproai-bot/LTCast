...
export function Transport({ onPlay, onPause, onStop, onSeek }: Props): React.JSX.Element {
  const {
    ...
    lang,
    tcGeneratorMode, setlist, activeSetlistIndex
  } = useStore()

  // Mirror the same effectiveGeneratorMode logic from App.tsx
  ...
  // Buttons are enabled if there's a file loaded OR generator mode is active
  const canPlay = !!duration || effectiveGeneratorMode
    ...
    <div className="transport">
      ...
      <div className="transport-row">
        <div className="transport-buttons">
          <button className="btn-transport btn-stop" onClick={onStop} disabled={!canPlay} title={t(lang, 'stop')}>⏹</button>

          {playState === 'playing' ? (
            <button className="btn-transport btn-play active" onClick={onPause} disabled={!canPlay} title={t(lang, 'pause')}>⏸</button>
          ) : (
            <button className="btn-transport btn-play" onClick={onPlay} disabled={!canPlay} title={t(lang, 'play')}>▶</button>
          )}
