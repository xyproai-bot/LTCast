...
export function TimecodeDisplay({ fullscreen }: Props): React.JSX.Element {
  const {
    ...
    setlist, activeSetlistIndex, setSetlistItemStartTC, setSetlistItemTcGeneratorMode
  } = useStore()

  const activeTrack = activeSetlistIndex !== null ? setlist[activeSetlistIndex] : null
  const effectiveStartTC = activeTrack?.startTC ?? generatorStartTC
  const effectiveGeneratorMode = activeTrack?.tcGeneratorMode ?? tcGeneratorMode

  const handleStartTCChange = (tc: string) => {
    if (activeSetlistIndex !== null) {
      setSetlistItemStartTC(activeSetlistIndex, tc)
    } else {
      setGeneratorStartTC(tc)
    }
  }

  const handleModeChange = (mode: boolean) => {
    if (activeSetlistIndex !== null) {
      setSetlistItemTcGeneratorMode(activeSetlistIndex, mode)
    } else {
      setTcGeneratorMode(mode)
    }
  }
    ...
    <div className={`tc-display${fullscreen ? ' tc-display--fullscreen' : ''}`}>
      ...
      {!fullscreen && (
        <div className="tc-mode-row">
          <button
            className={`tc-mode-btn${!effectiveGeneratorMode ? ' active' : ''}`}
      ...
      {!effectiveGeneratorMode && !fullscreen && (
