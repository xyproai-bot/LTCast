...
interface Window {
  api: {
    ...
    extractAudioFromVideoToFile(path: string): Promise<{ path: string; name: string }>
    generateLtcWav(opts: { startTC: string; durationSec: number; fps: number; amplitude?: number }): Promise<{ path: string; name: string }>
    ...
    // Save raw buffer to user-chosen file
    saveFileBuffer(defaultName: string, filters: { name: string; extensions: string[] }[], buffer: ArrayBuffer): Promise<string | null>
