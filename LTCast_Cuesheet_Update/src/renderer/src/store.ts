...
export interface SetlistItem {
  ...
  startTC?: string       // per-song start TC override; undefined = use global
  tcGeneratorMode?: boolean // per-song override for LTC Generator vs Reader mode
  nextTrackId?: string   // ID of the track to play next, or 'stop'/'next'. Null/undefined = fallback to autoPlayNext setting.
}

/** Sentinel path used to identify a TC-Generator-only setlist item (no audio file). */
export const TC_GENERATOR_PATH = '__tc_generator__'
export const isGeneratorItem = (item: SetlistItem): boolean => item.path === TC_GENERATOR_PATH
...
function buildPresetData(s: Pick<AppState,
  'lang' | 'rightTab' | 'offsetFrames' | 'loop' | 'autoPlayNext' | 'loopA' | 'loopB' | 'musicOutputDeviceId' |
  ...
  return {
    ...
    loop: s.loop, autoPlayNext: s.autoPlayNext, loopA: s.loopA, loopB: s.loopB,
...
export interface AppState {
  ...
  setSetlistItemStartTC: (index: number, startTC: string | undefined) => void
  setSetlistItemTcGeneratorMode: (index: number, mode: boolean | undefined) => void
  setSetlistItemNextTrackId: (index: number, nextTrackId: string | undefined) => void
  setSetlistItemName: (index: number, name: string) => void
...
export const useStore = create<AppState>()(persist((set): AppState => ({
  ...
  setAutoPlayNext: (autoPlayNext) => set({ autoPlayNext, presetDirty: true }),
  ...
  addToSetlist: (items) => set((s) => ({
    setlist: [...s.setlist, ...items
      .filter(item => item.path === TC_GENERATOR_PATH || !s.setlist.some(existing => existing.path === item.path))
  ...
  clearSetlist: () => {
    ...
    window.api.showConfirmDialog(
      ...
      set({
        previousSetlist: { setlist: current.setlist, activeIndex: current.activeSetlistIndex } as { setlist: SetlistItem[]; activeIndex: number | null },
        setlist: [],
        activeSetlistIndex: null,
        presetDirty: true
      })
  ...
  setSetlistItemOffset: (index, offsetFrames) => set((s) => {
    if (index < 0 || index >= s.setlist.length) return s
    const setlist = [...s.setlist]
  ...
  setSetlistItemStartTC: (index, startTC) => set((s) => {
    if (index < 0 || index >= s.setlist.length) return s
    const setlist = [...s.setlist]
    setlist[index] = { ...setlist[index], startTC }
    return { setlist, presetDirty: true }
  }),
  setSetlistItemTcGeneratorMode: (index, mode) => set((s) => {
    if (index < 0 || index >= s.setlist.length) return s
    const setlist = [...s.setlist]
    setlist[index] = { ...setlist[index], tcGeneratorMode: mode }
    return { setlist, presetDirty: true }
  }),
  setSetlistItemNextTrackId: (index, nextTrackId) => set((s) => {
    if (index < 0 || index >= s.setlist.length) return s
    const setlist = [...s.setlist]
    setlist[index] = { ...setlist[index], nextTrackId }
    return { setlist, presetDirty: true }
  }),
  setSetlistItemName: (index, name) => set((s) => {
    if (index < 0 || index >= s.setlist.length) return s
    const newName = name.trim()
    if (!newName) return s
    const setlist = [...s.setlist]
    setlist[index] = { ...setlist[index], name: newName }
    return { setlist, presetDirty: true }
  }),
